	...
	for time.Since(t0).Seconds() < 10 && ts.checkFinished() == false {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				// DPrintf("CommitedIdx: %d And Command: %d Replicated in %d servers but expected servers %d", index, cmd1, nd, expectedServers)
				for _, svr := range ts.srvs {
					// _, isLeader := svr.GetState()
					svr.mu.Lock()
					// DPrintf("Replicated in svr: %d Logs: %d is leader %v", svr.me, svr.logs, isLeader)
					svr.mu.Unlock()
				}
