...
func (rs *rfsrv) applier(applyCh chan raftapi.ApplyMsg) {
	for m := range applyCh {
		if m.CommandValid == false {
			...
			term, isLeader := rs.GetState()
			if m.CommandIndex > 1 && prevok == false {
				err_msg = fmt.Sprintf("server %v apply out of order %v logs %v and is leader %v with term %d", rs.me, m.CommandIndex, rs.logs, isLeader, term)
			}
			if err_msg != "" {
